...
# Run with
# /spark/bin/spark-submit --py-files tissue_detector.py,LSVM_tissue_bg_model_promort.pickle tiler_all.py
#
...
class Tiler():
    ...
    def get_coords(self):
        ...
        tiss = Image.open(self.mask_tissue_fn)
        ...
        mini_norm = np.array(norm.resize((nx, ny)))
        mini_tum = np.array(tum.resize((nx, ny)))
        ts = np.array(tiss.resize((nx, ny))) # tissue
        ts -= mini_norm + mini_tum # minus non-normal and non-tumor
        ze = np.zeros_like(ts)
        mini_tiss = np.maximum(ts, ze) # get rid of negative values
        # label choice
        lab_norm = 1  # 0b001
        lab_tum = 2   # 0b010
        lab_tiss = 4  # 0b100
        ...
        coords_tum = [((round(sc_x*x), round(sc_y*y)), lab_tum) for x in
                      range(nx) for y in range(ny)
                      if mini_tum[y,x]==1]
        coords_tiss = [((round(sc_x*x), round(sc_y*y)), lab_tiss) for x in
                      range(nx) for y in range(ny)
                      if mini_tiss[y,x]==1]
        norm.close(); tum.close(); tiss.close()
        return (coords_norm + coords_tum + coords_tiss)
...
def write_to_cassandra(password):
    def ret(items):
        ...
        cw = CassandraWriter(auth_prov, ['cassandra_db'],
                             f'promort.ids_cosk_all_{pyram_lev}',
                             f'promort.data_cosk_all_{pyram_lev}',
                             f'promort.metadata_cosk_all_{pyram_lev}',)
        cw.save_items(items)
    return(ret)
...
def run():
    ...
    parts_0 = 96 # get list of patches
    parts_1 = 2 # extract patches
    parts_2 = 2 # write to cassandra
